...
- `precomputations.circom`: `TACEO_PRECOMPUTATION_*` wrappers around Poseidon2 and circomlib templates
...
Known-answer test vectors for the sponge, UHF, and compression circuits live in `tests/kats/` and are generated by `python3 scripts/generate_kats.py`, which reimplements the primitives in pure Python and self-checks against the Poseidon2 permutation vectors before writing.
